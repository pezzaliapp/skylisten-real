...
l'euristica (peso 50/50, vedi `MODEL_WEIGHT` in `app/app.js`).
...
1. **Aggiungi TensorFlow.js in locale.** Scarica `tf.min.js` da una release
   ufficiale e mettilo, ad esempio, in `app/vendor/tf.min.js`. Poi caricalo in
   `app/index.html` **prima** dello script dell'app:
...
Il modulo `app/js/model.js` rileva la forma dell'input del modello:
...
- **Rank 4** `[1, n_mels, frames, 1]` — mel-spectrogram. È il formato prodotto
  ...
  L'app calcola il log-mel in tempo reale con `app/js/dsp.js`, replicando i
